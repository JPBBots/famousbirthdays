...
import { famousBirthdays } from '../fbApi'

@Command('famousbirthdays', 'Gets birthdays for today or specific day')
export class FamousBirthdaysCommand {
  @Run()
  async birthdays (
    @Options.String('date', 'Specific date to show') d?: string
  ) {
    const date = d == 'tomorrow' ? new Date(Date.now() + 86400000) : new Date(d || Date.now())
